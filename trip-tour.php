...
if (!$tour) {
    ...
    echo '<div class="wrap section"><p class="empty-note">That tour isn\'t available. <a href="' . h(url('/trip-tours.php')) . '">Browse all trip tours</a>.</p></div>';
    require __DIR__ . '/includes/site_footer.php';
    exit;
}

$destinations = db()->prepare('SELECT d.id, d.name, c.name AS country_name FROM trip_tour_destinations td JOIN destinations d ON d.id = td.destination_id JOIN countries c ON c.id = d.country_id WHERE td.trip_tour_id = ? ORDER BY d.name');
...
$mainGallery = get_media('trip_tour', $id);
$overviewGallery = get_media('trip_tour_overview', $id);
$hotelGallery = get_media('trip_tour_hotel', $id);
$vehicleGallery = get_media('trip_tour_vehicle', $id);
$flightGallery = get_media('trip_tour_flight', $id);
...
<header class="page-header">
  <div class="wrap">
    ...
    <p class="page-header__eyebrow page-header__eyebrow--categories"><?= h($tour['category_name']) ?> &middot; <?= h($tour['budget_type']) ?></p>
    <?php if ($destinations): ?>
    ...
    <div class="detail-grid">
        ...
        <?php if ($activities): ?>
          ...
          <div class="activity-list">
            <?php foreach ($activities as $activity):
              $activityGallery = get_media('trip_tour_activity', $activity['id']);
            ?>
      ...
      <aside class="detail-side">
        <div class="side-card side-card--price">
          <p class="side-card__permit-no">ITINERARY NO. TT&ndash;<?= str_pad((string) $tour['id'], 4, '0', STR_PAD_LEFT) ?></p>
          ...
          <ul class="side-card__facts">
            ...
            <?php
              $locationCountries = array_unique(array_column($destinations, 'country_name'));
            ?>
            ...
            <?php endif; ?>
            <li><span>Tour type</span><span><?= h($tour['category_name']) ?></span></li>
...
<section class="section section--savanna" id="enquire">
  <div class="wrap" style="max-width:640px;">
    ...
    <?php else: ?>
      <form class="site-form" method="post" action="<?= h(url('/book-trip.php')) ?>">
