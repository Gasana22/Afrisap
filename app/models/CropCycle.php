...
class CropCycle
{
    ...
    public static function paginated(int $page, int $perPage, int $organizationId): array
    {
        ...
        $countStmt = $pdo->prepare('SELECT COUNT(*) FROM crop_cycles cc
        ...
        $stmt = $pdo->prepare(self::SELECT_BASE . ' WHERE f.organization_id = :org_id ORDER BY cc.created_at DESC LIMIT :limit OFFSET :offset');
    ...
    /** The tenant-isolation check: fetching another organization's crop cycle by id returns null. */
    public static function findInOrganization(int $id, int $organizationId): ?array
    {
        $stmt = Database::connection()->prepare(self::SELECT_BASE . ' WHERE cc.id = :id AND f.organization_id = :org_id');
