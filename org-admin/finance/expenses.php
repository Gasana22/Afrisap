...
const EXPENSE_CATEGORIES = ['Inputs', 'Labor', 'Equipment', 'Utilities', 'Other'];
...
if (is_post() && ($_POST['_method'] ?? '') === 'DELETE' && csrf_verify()) {
    ...
    if ($old) {
        ...
        session_flash('success', 'Expense record deleted.');
    }
    redirect('org-admin/finance/expenses.php');
}
...
if (is_post() && ($_POST['_method'] ?? '') !== 'DELETE' && csrf_verify()) {
    ...
    if (!$errors) {
        ...
        session_flash('success', 'Expense transaction recorded.');
        redirect('org-admin/finance/expenses.php');
    }
}
...
$transactions = tenant_all(
    ...
    $orgId,
    "AND type = 'expense' $whereSql ORDER BY transaction_date DESC, id DESC",
    $params
);

$existingCategories = db_all(
    "SELECT DISTINCT category FROM financial_transactions WHERE organization_id = :org_id AND type = 'expense' ORDER BY category",
    ['org_id' => $orgId]
);

$monthlyTotal = (float) db_value(
    "SELECT COALESCE(SUM(amount), 0) FROM financial_transactions WHERE organization_id = :org_id AND type = 'expense' AND transaction_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01')",
    ['org_id' => $orgId]
);
...
render_header(['title' => 'Expenses', 'context' => 'org-admin']);
?>
    ...
    <main class="app-content">
      ...
      <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
          ...
          <p class="text-muted small mb-0">Track and review expense transactions for your organization.</p>
        </div>
        <a href="<?= base_url('org-admin/finance/income.php') ?>" class="btn btn-outline-secondary"><i class="bi bi-graph-up-arrow"></i> View Income</a>
      </div>

      <div class="row g-3 mb-4">
        <div class="col-sm-6 col-xl-4">
          <?php render_stat_card('This Month\'s Expenses', format_money($monthlyTotal), null, 'bi-graph-down-arrow', 'danger'); ?>
        </div>
        <div class="col-sm-6 col-xl-4">
        ...
        <form method="post" enctype="multipart/form-data">
          ...
          <div class="row">
            <div class="col-md-4 mb-3">
              ...
              <input type="text" name="category" class="form-control" list="expenseCategories" required value="<?= e($input['category']) ?>">
          ...
          <button type="submit" class="btn btn-primary"><i class="bi bi-plus-lg"></i> Save Expense</button>
        </form>
      </div>
      ...
      <div class="content-card mb-4">
        ...
        <form method="get" class="row g-2 align-items-end">
          ...
          <div class="col-md-3 d-flex gap-2">
            ...
            <a href="<?= base_url('org-admin/finance/expenses.php') ?>" class="btn btn-outline-secondary">Reset</a>
          </div>
        </form>
      </div>

      <div class="content-card">
        <div class="content-card-header"><h6>Expense Transactions</h6></div>
        <?php if (!$transactions): ?>
          <?php render_empty_state('No expense transactions recorded yet.', 'bi-cash-coin'); ?>
        <?php else: ?>
              ...
              <tbody>
                ...
                <?php foreach ($transactions as $t): ?>
                  <tr>
                    ...
                    <td class="text-end text-danger fw-semibold"><?= format_money((float) $t['amount']) ?></td>
                    ...
                    <td>
                      <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteExpense<?= $t['id'] ?>"><i class="bi bi-trash"></i></button>
                    </td>
                  </tr>
                  <?php confirm_delete_modal('deleteExpense' . $t['id'], base_url('org-admin/finance/expenses.php'), 'this expense transaction', ['id' => $t['id']]); ?>
                <?php endforeach; ?>
              </tbody>
            </table>
          </div>
