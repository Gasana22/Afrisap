...
if (is_post() && csrf_verify()) {
    ...
    if (!$errors) {
        ...
        $totalCost = $cost !== null ? $qty * $cost : null;
        ...
        audit_log($orgId, $currentUser['id'], 'update', 'inventory_items', $item['id'], ['quantity' => $item['quantity']], ['quantity' => (float) $item['quantity'] + $qty, 'stock_in' => $qty]);
        session_flash('success', 'Stock received: +' . number_format($qty, 2) . ' ' . ($item['unit'] ?: '') . ' added to ' . $item['name'] . '.');
        redirect('org-admin/inventory/stock-in.php?item_id=' . $item['id']);
    }
}
...
$recentTransactions = $item
    ? db_all("SELECT * FROM inventory_transactions WHERE item_id = :item_id AND type = 'stock_in' ORDER BY created_at DESC LIMIT 15", ['item_id' => $item['id']])
    : [];

render_header(['title' => 'Stock In', 'context' => 'org-admin']);
?>
      ...
      <div class="row g-3">
        <div class="col-lg-5">
          <div class="content-card">
            <h6 class="mb-3">Record Stock Receipt</h6>
            <form method="post">
              <?= csrf_field() ?>
              <div class="mb-3">
                <label class="form-label">Item</label>
                <select name="item_id" class="form-select" required onchange="window.location='<?= base_url('org-admin/inventory/stock-in.php') ?>?item_id='+this.value">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="reference_type" class="form-control" value="manual" placeholder="manual, purchase_order, ...">
              </div>
              <div class="mb-3">
                <label class="form-label">Notes</label>
                <textarea name="notes" class="form-control" rows="2"></textarea>
              </div>
              <button type="submit" class="btn btn-success w-100" <?= $item ? '' : 'disabled' ?>><i class="bi bi-box-arrow-in-down"></i> Record Stock In</button>
            </form>
          </div>
        </div>
        <div class="col-lg-7">
          <div class="content-card">
            <h6 class="mb-3">Recent Stock-In Transactions <?= $item ? 'for ' . e($item['name']) : '' ?></h6>
            <?php if (!$item): ?>
              <?php render_empty_state('Select an item to view its recent stock-in history.'); ?>
            <?php elseif (!$recentTransactions): ?>
              <?php render_empty_state('No stock-in transactions recorded yet for this item.'); ?>
            <?php else: ?>
              <div class="table-responsive">
                <table class="table-app">
                  <thead><tr><th>Date</th><th>Quantity</th><th>Unit Cost</th><th>Total Cost</th><th>Reference</th><th>Notes</th></tr></thead>
                  <tbody>
                  <?php foreach ($recentTransactions as $t): ?>
                    <tr>
                      <td><?= format_date($t['created_at'], 'd M Y H:i') ?></td>
                      <td class="text-success">+<?= number_format((float) $t['quantity'], 2) ?></td>
                      <td><?= $t['unit_cost'] !== null ? format_money((float) $t['unit_cost']) : '-' ?></td>
                      <td><?= $t['total_cost'] !== null ? format_money((float) $t['total_cost']) : '-' ?></td>
