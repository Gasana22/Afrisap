...
if (!$tourId || $name === '' || $email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    redirect('/trip-tour.php?id=' . $tourId);
}

$stmt = db()->prepare("SELECT id, title FROM trip_tours WHERE id = ? AND status = 'published'");
$stmt->execute([$tourId]);
$tour = $stmt->fetch();
if (!$tour) {
    redirect('/trip-tours.php');
...
db()->prepare('INSERT INTO bookings (bookable_type, bookable_id, customer_name, customer_email, customer_phone, travel_date, num_people, message, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
    ->execute(['trip_tour', $tourId, $name, $email, $phone, $travelDate, $numPeople, $message, 'pending']);
...
notify_admin(
    'New trip tour enquiry: ' . $tour['title'],
    "New trip tour enquiry\n\nTour: {$tour['title']}\nName: $name\nEmail: $email\nPhone: $phone\nTravel date: " . ($travelDate ?: 'Not specified') . "\nPeople: $numPeople\nMessage: $message"
);

redirect('/trip-tour.php?id=' . $tourId . '&sent=1');
