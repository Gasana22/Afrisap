...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($id) {
            db()->prepare('UPDATE tour_activities SET title = ?, description = ?, activity_id = ? WHERE id = ?')
            ...
            db()->prepare('INSERT INTO tour_activities (tour_id, title, description, activity_id, sort_order) VALUES (?, ?, ?, ?, ?)')
