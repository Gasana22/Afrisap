...
if ($id) {
    $stmt = db()->prepare('SELECT * FROM tour_itinerary_days WHERE id = ? AND tour_id = ?');
    ...
    $nextStmt = db()->prepare('SELECT COALESCE(MAX(day_number), 0) + 1 FROM tour_itinerary_days WHERE tour_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $dupStmt = db()->prepare('SELECT id FROM tour_itinerary_days WHERE tour_id = ? AND day_number = ? AND id != ?');
    ...
    if (!$errors) {
        if ($id) {
            db()->prepare('UPDATE tour_itinerary_days SET day_number = ?, title = ?, description = ? WHERE id = ?')
                ->execute([(int) $day['day_number'], $day['title'], $day['description'], $id]);
        } else {
            db()->prepare('INSERT INTO tour_itinerary_days (tour_id, day_number, title, description) VALUES (?, ?, ?, ?)')
    ...
    <form method="post" novalidate>
      ...
      <div class="form-grid">
        ...
        <div class="form-field form-field--full<?= isset($errors['title']) ? ' has-error' : '' ?>">
          ...
          <input type="text" id="title" name="title" value="<?= h($day['title']) ?>" placeholder="e.g. Gorilla trekking in Bwindi" required>
