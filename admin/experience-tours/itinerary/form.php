...
if ($id) {
    $stmt = db()->prepare('SELECT * FROM experience_tour_itinerary_days WHERE id = ? AND experience_tour_id = ?');
    ...
    $nextStmt = db()->prepare('SELECT COALESCE(MAX(day_number), 0) + 1 FROM experience_tour_itinerary_days WHERE experience_tour_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $dupStmt = db()->prepare('SELECT id FROM experience_tour_itinerary_days WHERE experience_tour_id = ? AND day_number = ? AND id != ?');
    ...
    if (!$errors) {
        if ($id) {
            db()->prepare('UPDATE experience_tour_itinerary_days SET day_number = ?, title = ?, description = ? WHERE id = ?')
                ->execute([(int) $day['day_number'], $day['title'], $day['description'], $id]);
        } else {
            db()->prepare('INSERT INTO experience_tour_itinerary_days (experience_tour_id, day_number, title, description) VALUES (?, ?, ?, ?)')
    ...
    <form method="post" novalidate>
      ...
      <div class="form-grid">
        ...
        <div class="form-field form-field--full<?= isset($errors['title']) ? ' has-error' : '' ?>">
          ...
          <input type="text" id="title" name="title" value="<?= h($day['title']) ?>" placeholder="e.g. Traditional dance and drumming" required>
