...
if ($id) {
    $stmt = db()->prepare('SELECT * FROM trip_tour_activities WHERE id = ? AND trip_tour_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($id) {
            db()->prepare('UPDATE trip_tour_activities SET title = ?, description = ?, activity_id = ? WHERE id = ?')
                ->execute([$activity['title'], $activity['description'], $activity['activity_id'], $id]);
        } else {
            $stmt = db()->prepare('SELECT COALESCE(MAX(sort_order), -1) + 1 FROM trip_tour_activities WHERE trip_tour_id = ?');
            $stmt->execute([$tourId]);
            $nextOrder = (int) $stmt->fetchColumn();
            db()->prepare('INSERT INTO trip_tour_activities (trip_tour_id, title, description, activity_id, sort_order) VALUES (?, ?, ?, ?, ?)')
                ->execute([$tourId, $activity['title'], $activity['description'], $activity['activity_id'], $nextOrder]);
        }
    ...
    <form method="post" novalidate>
      ...
      <div class="form-grid">
        <div class="form-field form-field--full">
          ...
          <span class="hint">Picking one fills the title/description below &mdash; still editable, and this tour keeps its own copy.</span>
        </div>
