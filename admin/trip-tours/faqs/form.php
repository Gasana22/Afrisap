...
if ($id) {
    $stmt = db()->prepare('SELECT * FROM trip_tour_faqs WHERE id = ? AND trip_tour_id = ?');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($id) {
            db()->prepare('UPDATE trip_tour_faqs SET question = ?, answer = ? WHERE id = ?')
                ->execute([$faq['question'], $faq['answer'], $id]);
        } else {
            $stmt = db()->prepare('SELECT COALESCE(MAX(sort_order), -1) + 1 FROM trip_tour_faqs WHERE trip_tour_id = ?');
            $stmt->execute([$tourId]);
            $nextOrder = (int) $stmt->fetchColumn();
            db()->prepare('INSERT INTO trip_tour_faqs (trip_tour_id, question, answer, sort_order) VALUES (?, ?, ?, ?)')
