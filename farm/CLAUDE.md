...
Multi-tenant farm management web app. **Classic flat-file PHP** — no framework, no router, no front controller. Deployed as a subfolder (e.g. `xampp/htdocs/farm/`, or `/farm` under a domain root), so pages are hit directly by URL (e.g. `localhost/farm/admin/farms.php`).
...
admin/
  includes/
    auth-check.php           Session guard — require this at the top of every protected admin page
    header.php                Sidebar nav (every module) + topbar with notification count. Set $pageTitle / $activePage before requiring.
...
database/
  ...
  seed.php                     Seed script — roles, permissions, demo org/users, crop types, a season
...
- **Multi-tenancy is load-bearing.** Every tenant-scoped query must filter by `organization_id = current_organization_id()`, *except* for platform users (`is_platform_user()` — role scope `platform`, `organization_id IS NULL`), who see across tenants. Use `farm_or_404()` for a single farm and `visible_farm_ids()` (with `in_placeholders()` for the `IN (...)` clause) for anything that spans farms — livestock, workers, inventory, finance, etc. all follow this. Getting this wrong is a cross-tenant data leak, not a cosmetic bug.
- **Permissions**: gate every mutating (POST) action with `require_permission('module.manage')` (checked against `role_permissions`/`permissions`) rather than checking `role_slug` directly. Reads stay open to any logged-in user, same as the rest of the admin panel. The permission codes are seeded in `database/seed.php`.
